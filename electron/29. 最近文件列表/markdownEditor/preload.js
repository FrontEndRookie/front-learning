...
contextBridge.exposeInMainWorld("electronAPI", {
   ...
   */
  on: (channel, listener) => {
    const allowedChannels = ["load", "format", "open-file"];
  ...
  /**
   *
   * @param {*} content markdown编辑器的内容
   */
  startDrag: (content) => {
    // 触发主进程的 ondragstart 事件，将 markdown 内容传递给主进程
    ipcRenderer.send("ondragstart", content);
  },
  addRecentDocument: (filePath) => {
    // 在这里，我们主要是触发主进程的事件
    ipcRenderer.send("add-recent-document", filePath);
  },
});
