<template>
 ...
 <div>
    <button v-for="(item,index) in data" :key="index" @click="target.comName=item.comName">{{item.name}}</button>
 </div>
...
import { ref, reactive, markRaw } from 'vue'
    ...
    const data = reactive<tabs[]>([{name:'a',comName:markRaw(A)},{name:'b',comName:markRaw(B)},{name:'c',comName:markRaw(C)}])
